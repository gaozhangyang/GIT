...
def matchY(Y_pred,Y_true):
    ...
    pred_set={} # the set of predicted classes, for example：pred_set[-1]={816,501}
    ...
    sort_idx=np.argsort(-np.array(list(map(len,pred_set.values())))) # sort the index of set according to its number of points
    
    Y_pred=np.zeros_like(Y_pred)-2 # initial with -2, representing the false predicted points
    for i in sort_idx:
        if len(true_set)==0:
            break
        pred_idx=list(pred_set.keys())[i]
        real_y=match(pred_set[pred_idx],true_set,noise_set) # find the real label of pred_set[pred_idx] from true_set
        if real_y is None:
